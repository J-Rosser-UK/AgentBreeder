    ...
    def __init__(self, session=None, **kwargs):
        ...
        for column in self.__table__.columns:
            ...
            if column_name in kwargs:
                self.validate_column_value(column_name, column_type, kwargs[column_name])
    ...
    def validate_column_value(self, column_name, column_type, value):
        ...
        if isinstance(column_type, String):
            if not isinstance(value, str):
                raise Exception(f"Invalid value for column '{column_name}'. Expected a string but got {type(value).__name__}.")
        elif isinstance(column_type, Float):
            if not isinstance(value, (float, int)):  # Allow integers since they can be cast to float
                raise Exception(f"Invalid value for column '{column_name}'. Expected a float but got {type(value).__name__}.")
        elif isinstance(column_type, JSON):
            if not isinstance(value, (dict, list, str, int, float, bool, type(None))):
                raise Exception(f"Invalid value for column '{column_name}'. Expected a JSON-compatible type but got {type(value).__name__}.")
        elif isinstance(column_type, ForeignKey):
            if not isinstance(value, str):  # Typically foreign keys are strings (UUID or other identifiers)
                raise Exception(f"Invalid value for foreign key column '{column_name}'. Expected a string but got {type(value).__name__}.")
        elif isinstance(column_type, DateTime):
            if not isinstance(value, (datetime.datetime, str)):  # Allow datetime or ISO 8601 string
                raise Exception(f"Invalid value for column '{column_name}'. Expected a datetime or ISO 8601 string but got {type(value).__name__}.")
        elif isinstance(column_type, Integer):
            if not isinstance(value, int):
                raise Exception(f"Invalid value for column '{column_name}'. Expected an integer but got {type(value).__name__}.")
        elif isinstance(column_type, Boolean):
            if not isinstance(value, bool):
                raise Exception(f"Invalid value for column '{column_name}'. Expected a boolean but got {type(value).__name__}.")
        else:
            raise Exception(f"Unsupported column type '{type(column_type).__name__}' for column '{column_name}'.")
