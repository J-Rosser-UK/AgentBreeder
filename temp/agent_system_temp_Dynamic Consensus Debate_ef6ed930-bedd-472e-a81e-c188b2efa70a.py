...
class AgentSystem:
    ...
    def forward(self, task: str) -> str:
        ...
        # Initialize debate agents with different roles and moderate temperature for varied reasoning
        ...
        while not consensus_reached:
            for i in range(len(debate_agents)):
                if i == 0:
                    ...
                    previous_thoughts = [chat.content for chat in meeting.chats if chat.agent in debate_agents[:i]]
                    noise = np.random.choice(['', ' (with some uncertainty)', ' (revised)'], p=[0.7, 0.2, 0.1])
                    meeting.chats.append(self.Chat(agent=system, content=f"Given solutions to the problem from other agents, consider their opinions as additional advice. Please think carefully and provide an updated answer. Reminder, the task is: {task}. Previous thoughts: {previous_thoughts} {noise}"))
                    output = debate_agents[i].forward(response_format={"thinking": "Your step by step thinking.", "response": "Your final response.", "answer": "A single letter, A, B, C, or D."})
