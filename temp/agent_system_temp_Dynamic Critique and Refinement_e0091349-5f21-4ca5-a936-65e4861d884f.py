...
class AgentSystem:
    ...
    def forward(self, task: str) -> str:
        ...
        # Setup meeting
        meeting = self.Meeting(meeting_name="dynamic_reflexion")
        ...
        for i in range(N_max):
            # Critics challenge the outputs
            for j, critic in enumerate(critics):
                meeting.chats.append(
                    self.Chat(
                        agent=critic, 
                        content=f"Critique the reasoning of {cot_agents[j].agent_name}: {outputs[j]["thinking"]}"
                    )
                )
            ...
            for j, cot_agent in enumerate(cot_agents):
                meeting.chats.append(
                    self.Chat(
                        agent=system, 
                        content="Given the critiques, refine your reasoning and try again: {task}"
                    )
                )
                output = cot_agent.forward(
                    response_format={
        ...
        # Select the final answer based on quality of outputs
