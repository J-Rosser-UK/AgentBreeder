...
class AgentSystem:
    ...
    def forward(self, task: str) -> str:
        ...
        # Setup meeting
        meeting = self.Meeting(meeting_name="optimized_adversarial_feedback_loop")
        ...
        for generation in range(generations):
            ...
            # Adversarial critique in a round-robin manner
            for i, adv_agent in enumerate(adversarial_agents):
                for j in range(population_size):
                    critique = adv_agent.forward(
                        response_format={
                            "critique": f"Critique Main Agent {j} output."
                        }
                    )
                    meeting.chats.append(
                        self.Chat(
            ...
            for i, output in enumerate(outputs):
                ...
                revised_output = main_agents[i].forward(
                    response_format={
                        "thinking": f"Given feedback from adversarial agents, revise your answer to the task: {task}, thinking step by step.",
                        "answer": "A single letter, A, B, C or D."
                    }
                )
